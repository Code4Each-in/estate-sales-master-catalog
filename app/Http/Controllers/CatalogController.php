...
class CatalogController extends Controller
{
    ...
    public function getCatlogs()
    {
       ...
       // dd($catalogs->toarray());
        echo json_encode($catalogs->toarray());
    ...
    public function catalogData()
    {
        ...
        $client = new Client([
            'base_uri' => 'https://staging.recollection.com/',
            'auth' => [
                'ck_cc99edc37f4e0b1a8f1e34864259991df0e41fea',  // Consumer Key
                'cs_cb4a6cfa35725212be33ab6d057aa22c6082c5a9',  // Consumer Secret
            ],
            // Set verify to true to enable SSL verification
            'verify' => false
        ]);

        try {
            $response = $client->request('GET', 'wp-json/wc/v2/products/' . $id);
            ...
            if ($response->getStatusCode() === 200) {
                ...
                return response()->json([
                    'error' => 'Failed to fetch data. Unexpected status code: ' . $response->getStatusCode()
